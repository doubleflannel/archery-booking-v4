...
def emphasize_lines(img, distances, estimatedRadius, telemetry=None, ring_radius=None):
    '''
    ...
    if ring_radius is not None:
        ...
        if telemetry is not None:
            telemetry['radial_ring_estimate'] = telemetry.get('radial_ring_estimate', 0.0)
    else:
        ...
        if telemetry is not None:
            telemetry['radial_ring_estimate'] = telemetry.get('radial_ring_estimate', 0.0) + (time.perf_counter() - ring_start)

    # zero out all pixels outside of the outer ring
    thresh_start = time.perf_counter()
    ...
    if telemetry is not None:
        telemetry['radial_threshold_morph'] = telemetry.get('radial_threshold_morph', 0.0) + (time.perf_counter() - thresh_start)
    ...
    if telemetry is not None:
        telemetry['radial_line_detect'] = telemetry.get('radial_line_detect', 0.0) + (time.perf_counter() - line_start)
