...
def ratio_match(matcher, queryDesc, train, ratio, mask=None):
    '''
    ...
    Parameters:
        ...
        {Numpy.array|None} mask - Optional mask (uint8) to restrict keypoint detection area
    ...
    train_keys, train_desc = matcher.detectAndCompute(train, mask)
    bf = cv2.BFMatcher(crossCheck=False)
    best_match = []
    
    if type(train_desc) != type(None) and type(queryDesc) != type(None):
        # apply ratio test
...
def calc_homography(queryKeys, trainKeys, matches):
    ...
    # Use a slightly tighter reprojection threshold for robustness
